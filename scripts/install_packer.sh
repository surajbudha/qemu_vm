#!/bin/bash
# Install packer on linux systems
...
if [[ "$os_family" == *"ubuntu"* || "$os_family" == *"debian"* ]]
  then
    echo "Installing packer on Ubuntu/Debian"
    ...
    packer -help
    echo "packer installation completed on Ubuntu/Debian"

elif [[ "$os_family" == *"redhat"* || "$os_family" == *"centos"* || "$os_family" == *"fedora"* ]]
  then
    echo "Installing packer on RedHat/CentOS/Fedora"
    sudo yum install -y yum-utils
    sudo yum-config-manager --add-repo https://rpm.releases.hashicorp.com/RHEL/hashicorp.repo
    sudo yum -y install packer
    packer -help
    echo "packer installation completed on RedHat/CentOS/Fedora"
else
